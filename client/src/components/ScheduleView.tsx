              ...
              <div className="flex-1 flex flex-wrap items-center gap-2">
                {room.isMaintenance ? (
                  ...
                  roomBookings.map((b) => (
                    <div
                      key={b.id}
                      className={`p-2 sm:px-2.5 sm:py-1 rounded-xl border text-xs shadow-2xs flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2 max-w-full ${
                        b.isCheckedIn
                    ...
                    >
                      <div className="flex items-center space-x-1.5 whitespace-nowrap shrink-0">
                        <Clock className={`w-3.5 h-3.5 ${b.isCheckedIn ? 'text-emerald-600' : 'text-blue-600'} shrink-0`} />
                        <span className="font-bold tracking-tight">
                          {format(new Date(b.startTime), 'HH:mm')} - {format(new Date(b.endTime), 'HH:mm')}
                        </span>
                        {b.isCheckedIn && (
                          <span className="text-[9px] bg-emerald-600 text-white font-bold px-1.5 py-0.5 rounded-full whitespace-nowrap ml-0.5">
                            ✓ {t.checkedInBadge}
                          </span>
                        )}
                      </div>
                      ...
                      <div className="flex items-center space-x-2 min-w-0">
                        <span className="font-medium truncate max-w-[140px] sm:max-w-[120px]">{b.title}</span>
                        {b.user && (
                          <span className="text-[10px] text-slate-500 flex items-center shrink-0">
                            <User className="w-2.5 h-2.5 mr-0.5" />
                            <span className="truncate max-w-[80px]">{b.user.name}</span>
                          </span>
                        )}
                      </div>
                    </div>
